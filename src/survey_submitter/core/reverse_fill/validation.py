...
def _check_column_mapping(
    *,
    ...
    if (
        question_type in CHOICE_TYPES | TEXT_TYPES
        ...
        _append_question_issue_and_plan(
            ...
            question_type=question_type,
            columns=columns,
            detail="这道题在 Excel 中对应了多列，V1 无法确认唯一答案列",
    ...
    if question_type == QuestionType.MATRIX:
        ...
        if row_texts and len(columns) != len(row_texts):
            _append_question_issue_and_plan(
                ...
                question_type=question_type,
                columns=columns,
                detail=f"矩阵题解析出 {len(row_texts)} 行，但 Excel 里只有 {len(columns)} 列",
    ...
    if question_type == QuestionType.MULTI_TEXT:
        ...
        if blank_labels and len(columns) != len(blank_labels):
            _append_question_issue_and_plan(
                ...
                question_type=question_type,
                columns=columns,
                detail=f"多项填空解析出 {len(blank_labels)} 个空，但 Excel 里只有 {len(columns)} 列",
                category="mapping_mismatch",
...
def _validate_and_collect_question(
    *,
    ...
    prereq_error = _validate_question_prerequisite(
...
def build_reverse_fill_spec(
    *,
    ...
    if available_rows <= 0:
        issues.append(
            _build_no_sample_issue(start_row=normalized_start_row, total_samples=total_samples)
        )
