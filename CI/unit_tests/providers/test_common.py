...
class ProviderCommonTests:
    ...
    def test_normalize_survey_provider_returns_default_for_unknown_value(self) -> None:
        assert normalize_survey_provider('unknown', default=SURVEY_PROVIDER_WJX) == SURVEY_PROVIDER_WJX
    ...
    def test_ensure_question_provider_fields_normalizes_provider_metadata(self) -> None:
        result = ensure_question_provider_fields({'provider': '  wjx  ', 'provider_question_id': ' question-7 ', 'provider_page_id': ' 1 ', 'provider_type': ' dropdown ', 'unsupported': 1, 'unsupported_reason': '  暂不支持  '})
    ...
    def test_ensure_questions_provider_fields_filters_invalid_items(self) -> None:
        result = ensure_questions_provider_fields([{'provider': 'wjx', 'provider_question_id': ' q1 '}, 'bad', {'provider': 'nope'}], default_provider=SURVEY_PROVIDER_WJX)
        assert len(result) == 2
