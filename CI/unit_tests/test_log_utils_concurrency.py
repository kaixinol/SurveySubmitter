...
class LogBufferHandlerConcurrencyTests:
    ...
    def test_emit_processes_records_asynchronously(self) -> None:
        ...
        handler.emit(
            logging.LogRecord(logger.name, logging.INFO, __file__, 10, "普通日志", (), None)
        )
    ...
    def test_emit_keeps_only_latest_records_when_capacity_is_reached(self) -> None:
        ...
        for message in ("第一条", "第二条", "第三条"):
            handler.emit(
                logging.LogRecord(logger.name, logging.INFO, __file__, 10, message, (), None)
            )
    ...
    def test_emit_filters_sensitive_and_noise_messages(self) -> None:
        ...
        handler.emit(
            logging.LogRecord(
                logger.name, logging.INFO, __file__, 10, "Authorization: Bearer abc", (), None
            )
        )
        handler.emit(
            logging.LogRecord(
        ...
        with patch(
            ...
            with patch.object(
                ...
                side_effect=[
                    ...
                    original_format(
                        logging.LogRecord(
                            logger.name, logging.INFO, __file__, 10, "恢复后的日志", (), None
                        )
                    ),
                ],
            ):
                handler.emit(
                    logging.LogRecord(logger.name, logging.INFO, __file__, 10, "坏日志", (), None)
                )
                handler.emit(
                    logging.LogRecord(logger.name, logging.INFO, __file__, 10, "好日志", (), None)
                )
                assert self._wait_until(
                    lambda: any(("恢复后的日志" in entry.text for entry in handler.get_records()))
                )
    ...
    def test_emit_is_safe_under_multiple_threads(self) -> None:
        ...
        def _worker(idx: int) -> None:
            ...
            handler.emit(
                logging.LogRecord(
                    "unit.logbuffer.concurrent", logging.INFO, __file__, 10, f"日志-{idx}", (), None
                )
            )

        threads = [
            threading.Thread(target=_worker, args=(idx,), name=f"Logger-{idx}") for idx in range(5)
        ]
    ...
    def test_log_buffer_notifies_listener_after_batch_processed(self) -> None:
        ...
        try:
            handler.emit(
                logging.LogRecord(
                    "unit.logbuffer.listener", logging.INFO, __file__, 10, "监听日志", (), None
                )
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                record = logging.LogRecord(
                    "unit.logfile.async", logging.INFO, __file__, 10, "异步落盘", (), None
                )
                handler.emit(record)
                handler.flush()
                assert self._wait_until(
                    lambda: (
                        os.path.exists(path)
                        and "异步落盘" in open(path, "r", encoding="utf-8").read()
                    )
                )
            finally:
                handler.close()
