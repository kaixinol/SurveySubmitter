...
class ProviderCommonTests:
    def test_provider_run_context_uses_explicit_plan_and_resets_persona(self) -> None:
        ...
        with (
            patch(
                "survey_submitter.core.engine.provider_common.generate_persona",
                return_value={"name": "p"},
            ),
            patch(
                "survey_submitter.core.engine.provider_common.set_current_persona"
            ) as set_persona_mock,
            patch(
                "survey_submitter.core.engine.provider_common._reset_answer_context"
            ) as reset_context_mock,
            patch(
