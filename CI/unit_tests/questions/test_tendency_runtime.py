...
class TendencyRuntimeTests:
    ...
    def test_get_tendency_index_returns_plan_choice_when_distribution_locked(self) -> None:
        ...
        assert (
            tendency.get_tendency_index(5, [0, 0, 1, 0, 1], psycho_plan=plan, question_index=2) == 4
        )

    def test_get_tendency_index_blends_plan_choice_and_keeps_zero_weight_guard(
        self, patch_attrs
    ) -> None:
        ...
        assert (
            tendency.get_tendency_index(
                4, [0, 0, 5, 0], psycho_plan=plan, question_index=2, row_index=1
            )
            == 2
        )
    ...
    def test_blend_psychometric_choice_limits_to_window_and_positive_weights(
        self, patch_attrs
    ) -> None:
        patch_attrs(
            (
                ...
                "get_reliability_profile",
                lambda: SimpleNamespace(
                    consistency_outside_decay=0.2,
                    consistency_window_ratio=0.25,
                    consistency_window_max=1,
                    consistency_center_weight=3.0,
                    consistency_edge_weight=1.0,
                ),
            ),
    ...
    def test_psychometric_plan_errors_fall_back_to_regular_logic(self, patch_attrs) -> None:
        ...
        assert not tendency._is_distribution_locked_plan(
            _Plan({(1, None): 0}, fail_locked=True), 1, None
        )
