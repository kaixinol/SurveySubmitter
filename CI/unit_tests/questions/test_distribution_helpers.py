...
class _FakeCtx:
    ...
    def append_pending_distribution_choice(
        self, stat_key: str, option_index: int, option_count: int
    ) -> None:
...
class DistributionHelperTests:
    ...
    def test_resolve_probabilities_returns_target_when_no_runtime_context(
        self, monkeypatch
    ) -> None:
        monkeypatch.setattr(
            distribution,
            "normalize_dropdown_probs",
            lambda probabilities, option_count: [0.5, 0.5][:option_count],
        )

        result = distribution.resolve_probabilities([1, 1], 2, None, 5)
    ...
    def test_resolve_probabilities_applies_runtime_correction(self, monkeypatch) -> None:
        monkeypatch.setattr(
            distribution,
            "normalize_dropdown_probs",
            lambda probabilities, option_count: [0.7, 0.3][:option_count],
        )
        ctx = _FakeCtx(counts=(10, [9, 1]))

        result = distribution.resolve_probabilities([7, 3], 2, ctx, 8)  # ty:ignore[invalid-argument-type]
    ...
    def test_resolve_probabilities_uses_priority_profile_when_dimension_or_plan_active(
        self, monkeypatch
    ) -> None:
        monkeypatch.setattr(
            distribution,
            "normalize_dropdown_probs",
            lambda probabilities, option_count: [0.6, 0.4][:option_count],
        )
        monkeypatch.setattr(
            distribution,
        ...
        result = distribution.resolve_probabilities(
            [6, 4],
            2,
    ...
    def test_record_pending_choice_ignores_invalid_input_and_records_valid_choice(
        self,
    ) -> None:
        ctx = _FakeCtx()

        distribution.record_pending_choice(ctx, None, 0, 2)  # ty:ignore[invalid-argument-type]
        distribution.record_pending_choice(ctx, 1, -1, 2)  # ty:ignore[invalid-argument-type]
        distribution.record_pending_choice(ctx, 1, 5, 2)  # ty:ignore[invalid-argument-type]
        distribution.record_pending_choice(ctx, 1, 1, 2, row_index=3)  # ty:ignore[invalid-argument-type]
